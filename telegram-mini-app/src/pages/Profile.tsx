...
export default function Profile({ user }: { user: any }) {
  ...
  const avatar =
    ...
    tgUser?.photo_url ||
    `https://ui-avatars.com/api/?name=${user?.firstName || "M"}&background=random`;
    ...
    <div className="space-y-6 pb-6">
      <div className="flex flex-col items-center pt-8 pb-4">
        ...
        <h2 className="text-2xl font-black text-white mt-4">
          {user?.firstName || tgUser?.first_name || "Nomsiz Oylinchi"}
        </h2>
        <p className="text-hint text-sm font-medium mt-1">
          Siz biz bilan {joinDate} dan beri birgasiz
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-secondaryBg rounded-3xl p-5 border border-hint/10">
          ...
          <div className="text-2xl font-black text-white">
            {maxWpm}{" "}
            <span className="text-sm text-hint uppercase font-bold tracking-wider">WPM</span>
          </div>
        ...
        <div className="bg-secondaryBg rounded-3xl p-5 border border-hint/10">
          ...
          <div className="text-2xl font-black text-white">
            {compsParticipated}{" "}
            <span className="text-sm text-hint uppercase font-bold tracking-wider">ta</span>
          </div>
