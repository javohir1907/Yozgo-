...
export default function Home({ user }: { user: any }) {
  ...
  const handleRegister = async () => {
    try {
      ...
      if (tg) {
        tg.showConfirm(
          `${activeComp.title} ga ro'yxatdan o'tmoqchimisiz?`,
          async (confirmed: boolean) => {
            if (confirmed) {
              try {
                const r = await fetch(`${API_URL}/competitions/${activeComp.id}/register`, {
                  method: "POST",
                });
    ...
    <div className="space-y-6">
      <div className="flex justify-center py-4">
        <h1 className="text-3xl font-black text-white">
          YOZG<span className="text-primary">O</span>
        </h1>
      </div>
        ...
        {activeComp ? (
          ...
          <div className="bg-secondaryBg rounded-3xl p-8 text-center border border-hint/10">
            ...
            <p className="text-hint text-sm">
              Yangi musobaqalar haqida tez orada e'lon beramiz. Kuzatib boring!
            </p>
          </div>
        )}
              ...
              >
                ...
                <div className="p-4 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-hint line-clamp-1">
                      {ad.description || "Homiylarimizga tashrif buyuring"}
                    </p>
                  </div>
