...
export default function Competitions({ user }: { user: any }) {
  ...
  const handleRegister = async (comp: any) => {
    try {
      ...
      if (tg) {
        tg.showConfirm(
          `${comp.title} ga ro'yxatdan o'tmoqchimisiz?`,
          async (confirmed: boolean) => {
            if (confirmed) {
              try {
                const r = await fetch(`${API_URL}/competitions/${comp.id}/register`, {
                  method: "POST",
                });
          ...
          return (
            <div key={comp.id} className="bg-secondaryBg rounded-2xl p-5 border border-hint/10">
              <div className="flex justify-between items-start mb-4">
                ...
                <span
                  className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider ${statusColor}`}
                >
                  {status}
                </span>
              </div>
              ...
              {status === "aktiv" || status === "kutilmoqda" ? (
                ...
                <button
                  disabled
                  className="w-full bg-hint/5 text-hint/50 py-2.5 rounded-xl font-bold text-sm"
                >
                  Tugallangan
                </button>
              )}
