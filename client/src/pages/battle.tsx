    ...
    if (inputCode.trim()) {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            "Tarmoq yoki server bilan aloqa ulanmadi (Xatolik 500). Iltimos birozdan so'ng qayta urinib ko'ring."
          );
        }
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <Button onClick={() => (window.location.href = "/auth")}>{t.nav.signIn}</Button>
      </div>
    );
  }
          ...
          <Card className="border-2 hover:border-primary/50 transition-colors bg-card/50 backdrop-blur">
            ...
            <CardContent className="space-y-6">
              <p className="text-muted-foreground mb-2">
                Oldin jang sozlamalarini tasdiqlang, so'ngra xonangizni oching va do'stlaringizni
                taklif qiling.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    <option value="uz" className="bg-background text-foreground">
                      🇺🇿 O'zbekcha
                    </option>
                    <option value="en" className="bg-background text-foreground">
                      🇺🇸 English
                    </option>
                    <option value="ru" className="bg-background text-foreground">
                      🇷🇺 Русский
                    </option>
                  </select>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Slider
                    value={[testDuration]}
                    onValueChange={([v]) => setTestDuration(v)}
                    min={15}
                    max={60}
                    step={5}
                  />
              ...
              <Button
                ...
                disabled={isCreating}
                className="w-full h-12 text-lg font-bold mt-4 shadow-lg"
              >
          ...
          <Card className="border-2 hover:border-primary/50 transition-colors bg-card/50 backdrop-blur">
            ...
            <CardContent>
              <div className="flex gap-2">
                ...
                />
                <Button onClick={joinBattle} className="h-12 px-8 font-bold">
                  Qo'shilish
                </Button>
              </div>
            </CardContent>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3">
            <Badge
              variant="outline"
              className="text-primary border-primary px-3 py-1 font-mono text-lg"
            >
              {battleCode}
            </Badge>
            {isAdmin && (
                ...
                {isAdmin ? (
                  ...
                  <div className="flex flex-col items-center py-12 text-center">
                    ...
                    <p className="text-muted-foreground">
                      O'yinchilar: {room?.players?.length || 0}
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          {battleStart && !battleEnd && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Card
                  className={`p-4 border-2 flex flex-col items-center justify-center transition-colors ${isAttemptActive ? "bg-orange-500/10 border-orange-500/40" : "bg-card border-dashed"}`}
                >
                  <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">
                    Sizning Urinishingiz
                  </span>
                  <span
                    className={`text-4xl font-black font-mono focus-ring ${isAttemptActive ? "text-orange-500" : "text-muted-foreground"}`}
                  >
              ...
              <div className="relative">
                {!isAttemptActive ? (
                  <div className="absolute inset-0 z-10 bg-background/60 backdrop-blur-[2px] rounded-xl flex items-center justify-center flex-col gap-4 border-2 border-dashed border-primary/40 p-8 text-center">
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Har bir urinish {battleStart.settings.testDuration} soniya davom etadi.
                      </p>
                    </div>
                    <Button
                      onClick={startAttempt}
                      size="lg"
                      className="h-14 px-12 text-xl font-black rounded-full shadow-xl"
                      disabled={totalTimer === 0}
                    >
                      URINISHNI BOSHLASH
                    </Button>
                  </div>
                ...
                <div
                  className={`${!isAttemptActive ? "blur-sm opacity-30 select-none grayscale" : "animate-in fade-in duration-700"}`}
                >
                  <div className="flex justify-center gap-12 mb-8">
                    ...
                    <div className="text-center">
                      <div className="text-5xl font-black font-mono text-orange-500">
                        {accuracy}%
                      </div>
                      <div className="text-xs text-muted-foreground uppercase tracking-wider font-bold">
                        Aniqlik
                  ...
                  {battleEnd.results.map((player: any, index: number) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-lg ${index === 0 ? "bg-yellow-500 text-white" : "bg-muted text-muted-foreground"}`}
                        >
                          {index + 1}
                        </div>
                        <div>
                      ...
                      <div className="text-right flex flex-col items-end">
                        ...
                        <div className="text-sm font-bold text-orange-500 flex items-center">
                          {player.bestAccuracy ?? 100}% Accuracy
                        </div>
                      </div>
                    </motion.div>
                  ))}
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center -mt-4 px-1 pb-1">
                          <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
                            {player.attempts} Urinish
                          </span>
                          <span className="flex items-center gap-2">
                            <span className="text-xs font-black text-primary/60">
                              {player.bestWpm} Peak WPM
                            </span>
                            <span className="text-xs font-black text-orange-500/80">
                              {player.bestAccuracy ?? 100}% ACC
                            </span>
                          </span>
                        </div>
                      </motion.div>
