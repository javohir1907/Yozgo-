...
async def check_code_status(message: Message, state: FSMContext):
    ...
    if result:
        ...
        room_status = result.get('roomStatus', "Noma'lum")
        is_used_text = 'Ishlatilmagan' if not result.get('isUsed') else 'Ishlatilgan'
        is_expired_text = 'Ha' if result.get('isExpired') else "Yo'q"

        msg = (
            f"📊 <b>Kod ma'lumotlari:</b>\n"
            f"🎟️ Kod: <code>{code}</code>\n"
            f"🔢 Holati: {status_emoji} {is_used_text}\n"
            f"🏁 Xona holati: <code>{room_status}</code>\n"
            f"👥 Limit: {result.get('maxParticipants')} ta\n"
            f"📅 Muddati: {result.get('expiresAt')}\n"
            f"🚫 Expired: {is_expired_text}"
        )
        await message.answer(msg, reply_markup=paid_rooms_menu_kb())
    else:
