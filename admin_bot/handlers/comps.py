...
# --- MUSOBAQA YARATISH (FSM) QISMI O'ZGARISHSIZ ---
...
async def comp_end_time(message: Message, state: FSMContext):
    try:
        ...
        if response: await msg.edit_text("✅ <b>Musobaqa e'lon qilindi!</b>")
        else: await msg.edit_text("❌ Musobaqa yaratishda xatolik.")
        await state.clear()
    except ValueError:
...
async def render_comps_page(message_or_call, page: int):
    ...
    PER_PAGE = 5
    total_pages = math.ceil(len(comps_data) / PER_PAGE)
    ...
    for i, comp in enumerate(page_comps, start=1):
        ...
        text += f"🟢 Boshlanish: {comp.get('startTime')[:16].replace('T', ' ')}\n\n"
